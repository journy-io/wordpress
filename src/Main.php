...
final class Main {
	...
	private function initGeneralActions() {
		...
		add_action(
			'wp_ajax_' . $this->plugin->name . '_dismiss_dashboard_notices', array(
				&$this,
	...
	private function initCF7Action() {
		if ( $this->isCF7ed ) {
			add_action(
				"wpcf7_before_send_mail", function () {
	...
	public function registerSettings() {
		...
		register_setting(
			$this->plugin->name, 'jio_cf7_first_name_id', [
				'type'    => 'trim',
				'default' => 'first_name'
			]
		);
	...
	public function adminPanelsAndMetaBoxes() {
		add_submenu_page(
			'options-general.php', $this->plugin->displayName, $this->plugin->displayName,
	...
	public function outputHeaderSnippet() {
		if ( ! $this->body_open_supported ) {
			if ( $this->isMooveGdpr ) {
				add_action(
					'moove_gdpr_third_party_header_assets', function ( $scripts ) {
	...
	public function outputBodySnippet() {
		if ( $this->body_open_supported ) {
			if ( $this->isMooveGdpr ) {
				add_action(
					'moove_gdpr_third_party_body_assets', function ( $scripts ) {
					$scripts .= $this->outputSnippet();
